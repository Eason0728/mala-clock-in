...
`clock.html` 的背景會依**光復店當下的天氣**自動換情境：下雨就下雨、晴天就出太陽、雷雨會打閃電、入夜換月亮星星。原本的紅底＋火焰影片保留，只是依天氣調亮暗（雨天壓暗、晴天最亮），字卡與按鈕都在效果之上，不影響閱讀與點擊。

| 天氣 | 畫面 |
|---|---|
| 晴（白天） | 右上角大太陽，光芒緩慢旋轉、光暈呼吸，整片鋪暖金光 |
| 晴（夜間） | 月亮＋會閃爍的星星，畫面壓深 |
| 多雲／陰 | 雲團橫向飄移，天空轉灰（多雲白天仍看得到太陽） |
| 霧 | 大片灰白霧氣貼著畫面飄，對比降低 |
| 雨 | 白色雨絲斜落（遠近三層、有風向），底色壓成冷藍 |
| 雷雨 | 雨勢加大、風更斜，每 5～16 秒閃一次閃電 |
...
- 動畫只在頁面可見時跑，切到背景或鎖屏立即停。
- 粒子數量依螢幕大小計算並設上限（雨滴最多 200、雪花 120、星星 110），畫布解析度最高 2 倍。
- 手機開了「減少動態效果」（iOS 輔助使用／Android 無障礙）時，只畫靜止的一幀，不跑動畫。
- 全部效果都是 CSS＋canvas 畫的，**沒有新增任何圖片或影片檔**，不會增加載入量。
...
程式都在 `clock.html` 裡：CSS 在 `/* ── 天氣動態背景 ── */` 註解那段，JS 在檔尾最後一個 `<script>`（獨立 IIFE，與打卡邏輯完全分開，改壞了不會影響打卡）。

- 換門市座標 → 改該段開頭的 `STORE_LAT`／`STORE_LNG`
- 天氣多久重抓 → `CACHE_MS`（預設 20 分鐘）
- 雨太大／太小 → `buildParts()` 裡的密度倍率，或 `sceneOf()` 各代碼的 `power`
- 火焰要更亮或更暗 → CSS 最後那組 `body.wx-xxx .fireVid { opacity: … }`
