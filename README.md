...
- 比對規則：每個「輸入時段」找重疊最大的一個「打卡段」（只比對打卡 in/out 都完整的段）：
  ...
  - **反向：中間有間隔的兩個輸入時段配到同一條打卡段 → 標記「少刷N組卡」（2026-09-21 新增）。** 起因是許正昊 9/21：一天兩段班（11:00–14:30、17:00–21:45）只刷了第一段的上班卡與第二段的下班卡，中間兩張都沒刷。`pairShifts` 是「一張 in 配 16 小時內的下一筆 out」，這兩張在它眼裡就是完整的一長段，`unmatchedIns`／`unmatchedOuts` 都是空的 → **整天一個忘刷卡都標不出來、參考時數也不留白**（實測參考 10.25H vs 核定 8.25H，差 2 小時而表上全無紅字）。唯一看得出破綻的地方就是核定比對：兩個明明中間有間隔的時段，重疊最大的打卡段竟然是同一條。
    ...
    - ⚠⚠ **該店「規定不打卡的休息帶」造成的空檔不算**（`CONFIG.NO_PUNCH_BREAK_START`／`NO_PUNCH_BREAK_END`，2026-09-22 補）：目前只有**央廚 12:00–13:00**。央廚同仁照規定就是不刷中午那組卡，而核定頁的預填本來就把休息帶挖成缺口＝**每天都是兩段** → 不排除的話，央廚每一位、每一個上班日都會被標「少刷1組卡」，異常筆數天天+1、全勤的忘刷次數每天+2（形同全店全勤歸零）。判準是「空檔與休息帶有重疊就當休息」，休息帶以外的空檔照抓。
    - ⚠ **不可以改用「空檔短於 N 分鐘就不算」來閃這件事**：央廚休息是 60 分，許正昊漏刷的那一格（14:30–15:30）也是 60 分，長度上完全分不開——只有「這家店規定要不要刷」分得開，所以判準一定是門市設定，不是時間長度。
    - ⚠ **`CONFIG.NO_PUNCH_BREAK_*` 要與 `tools/build-store-pages.py` 的 `mgr_break` 同一組值**（前者管「這個缺口算不算漏刷」、後者管「核定頁預填要不要挖缺口」）。`tools/verify-store-backends.py` 的 `STORE_BREAK` 會逐店比對，改一邊忘了另一邊會被擋下來。
