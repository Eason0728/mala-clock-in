...
- 班段＝`ok` 事件依時間配對（in 配 12 小時內的下一筆 out），跨夜班顯示 `HH:MM–HH:MM(+1)` 並歸上班那天。配不到的顯示 `？`＋備註「下班忘刷卡」／「上班忘刷卡」；**當天只要有任一筆忘刷卡，該日參考時數整天留空白**（待人工判定，核定時數不受影響，見下）。
- 兩個時數欄意義不同（2026-07-13 值班主管核定上線後改版）：
  - **參考時數**＝打卡時間原始相減（1 位小數，不取整），純粹反映機器記錄，不受人工核定影響。
  - **核定時數**＝改讀 `approved` 分頁（值班主管在 `manager.html` 輸入的實際上班時段加總），**不再是** 15 分鐘取整的自動換算（`approvedHoursOfShift` 函式仍保留給 `whoami`/`my_recent` 的「今日時數」/「最近 40 天」用，但月表核定欄不再呼叫它）。同一 (date,emp_id) 若主管核定多次，取 `entered_at` 最新一筆；當天完全沒有核定紀錄則顯示「待核定」。核定不受打卡是否忘刷卡影響——即使當天忘刷卡，只要主管有輸入實際時段照樣顯示核定時數。
...
- 之後每 10 分鐘會自動檢查 `events`**與 `approved`** 兩個分頁有沒有新增列，任一張表有新列才重算當月月表；兩張表都沒新增就直接跳過，半夜幾乎零成本。也就是說：**同仁打卡後，或值班主管核定後，月表最多晚 10 分鐘反映**，不用再等隔天 05:00。
...
- `mgr_day {mgr_key, date?}`（date 缺省＝今天，Asia/Taipei）→ 驗證主管金鑰後回傳當天有打卡事件的每位同仁：`{emp_id, name, segments, reference, approved?}`，`approved` 只在已有核定紀錄時出現（`periods, approved_hours, status_text, manager_name`）。
...
出勤月表每人每日呈現三欄（詳見上方「出勤月表」章節）：**參考時數**（打卡原始相減，不受核定影響）｜**核定時數**（讀 `approved` 分頁最新一筆，無紀錄顯示「待核定」）｜**狀態**（既有忘刷卡/新裝置/超出範圍/請假註記，加上主管核定帶回的正常/遲到/早退等）。既有 15 分鐘取整的核定欄（`approvedHoursOfShift` 函式）已退出月表與同仁打卡頁，只留給 `whoami`/`my_recent` 的內部計算用。
