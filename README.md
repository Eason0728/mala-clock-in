...
- **異常筆數的口徑（2026-08-19 Eason 定義）**：計入**上班忘刷卡／下班忘刷卡／遲到／早退／病假／事假**，加上沿用的**新裝置待核准**（`ABNORMAL_PREFIXES`）。
  - **比對方式是「開頭相符」不是完全相等**，因為這些字串帶變動尾巴：`遲到5分`／`早退12分`／`病假2.5h`。主管核定的狀態是多項用「、」串成一個字串（`遲到5分、早退12分`），`countNoteMatches()` 會先拆開再逐項比——同一天遲到又早退算 **2 筆**。
  ...
  - **「超出範圍嘗試」不計數但仍標紅字**（`HIGHLIGHT_PREFIXES` ＝ `ABNORMAL_PREFIXES` 再加這一項）：那多半是同仁人在店裡但手機定位飄掉，一天連按十幾次就把數字灌爆（王禹婕 8/11 一天 12 次），當異常看會失真；但仍要看得到當天被擋過。標紅字用的是**已含核定狀態**的 `statusNotes`，跟計數看同一份資料。
  - 改動只影響統計欄與紅字，**不影響打卡判定**。
