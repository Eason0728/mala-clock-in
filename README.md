...
- **異常筆數的口徑（2026-08-19 起）**：只計「上班忘刷卡／下班忘刷卡／新裝置待核准」（`ABNORMAL_NOTES`），**不計「超出範圍嘗試」**——那多半是同仁人在店裡但手機定位飄掉，一天連按十幾次就把數字灌爆（王禹婕 8/11 一天 12 次），當異常看會失真。超出範圍嘗試仍會顯示在狀態欄、也仍標紅字（`HIGHLIGHT_NOTES` ＝ `ABNORMAL_NOTES` 再加這一項），看得到但不進統計。改動只影響統計欄，不影響打卡判定。
