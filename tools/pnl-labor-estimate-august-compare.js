 ...
 * 分別跑兩條路徑」的方式做等價比對：
 *
 *   路徑 A（模擬『鎖定本月』當下會算出的定案數字）：直接用**完整月份、不截止**的 payCollect
 *          （不傳 cutoffDate）＋原始員工物件（不套虛擬離職日）呼叫 payCalcOne——這就是
 *          handlePayrollCalc／costTotals()／costStable() 背後的同一條路徑（T14 已證明
 *          costStable() 與 costTotals() 的算法完全一致，這裡不重複證那段，只借用同一套
 *          payCalcOne 呼叫方式代表「定案數字」）。
 ...
 * 沒有排除任何一天。這支腳本就是要實測「確實逐鍵相同」，做為「已上線後、8 月真的鎖定時，
 * 用 pnlLaborEstimate 重算應該跟 pnlPayroll 讀到的定案快照對得起來」這件事的替代驗證
 * （README 有記錄這個侷限與正式上線後 Eason 可以怎麼補做一次真的對比）。
 *
...
function wireRead(sb) {
  vm.runInContext(`
    payRead = function(kind){
      var d = ${JSON.stringify({ master: MASTER, config: CONFIG_ROWS, holiday: HOLIDAY_ROWS, bonus: BONUS_ROWS })};
      return (d[kind] || []).slice();
    };
...
const resultA = vm.runInContext(`
  (function(){
    ...
    var att = payCollect(ym, cfg.meal_min_hours, st, holDates);   // 不傳 cutoffDate＝整月，模擬定案當下
    var bonusBy = {};
...
console.log('\n' + (diffCount ? `❌ 有 ${diffCount} 個鍵不一致` : '✅ 整月情境下，pnlLaborEstimate 與定案路徑逐鍵完全相同'));
